...
class Process:
    ...
    @staticmethod
    def __generate_average_filter(fshape):
    ...
    @staticmethod
    def __generate_sharpening_filter(fshape):

        # 차이가 적은 방법
        mask = np.zeros(fshape)
        mask[fshape[0] // 2, fshape[1] // 2] = 2
        mask = mask - Process.__generate_average_filter(fshape)
    ...
    def average_filtering(src):
        ...
        cv2.createTrackbar('Blurrity', 'Average Filter', 0, 20, Process.__trackbar_change)
        blurrity = 0
    ...
    def sharpening_filtering(src):
        ...
        cv2.createTrackbar('Sharpness', 'Sharpening Filter', 0, 100, Process.__trackbar_change)
        sharpness = 0
        ...
        while True:
            ...
            sharpening_filter = Process.__generate_sharpening_filter((filter_size, filter_size))
    ...
    def bilateral_filtering(src):
        ...
        cv2.createTrackbar('Sigma Color', 'Bilateral Filter', 0, 30, Process.__trackbar_change)
        cv2.createTrackbar('Sigma Space', 'Bilateral Filter', 0, 30, Process.__trackbar_change)
        sigma_color = 0
        sigma_space = 0
    ...
    def median_filtering(src):
        ...
        cv2.createTrackbar('Smoothness', 'Median Filter', 0, 10, Process.__trackbar_change)
        smoothness = 0
    ...
    def HSV_Adjustment(src):
        ...
        cv2.createTrackbar('Hue', 'HSV Adjustment', 0, 500, Process.__trackbar_change)
        cv2.createTrackbar('Saturation', 'HSV Adjustment', 0, 500, Process.__trackbar_change)
        cv2.createTrackbar('Value', 'HSV Adjustment', 0, 500, Process.__trackbar_change)
